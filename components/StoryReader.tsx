...
function StoryLine({
  ...
  function handlePointerUp(e: React.PointerEvent) {
    ...
    if (lo === hi) {
      // Plain tap, no drag — hand off the Intl.Segmenter span + offset; the
      // popup hook resolves it against CEDICT itself. Skipped only on a real
      // desktop+extension page (deferToExtension) — everywhere else (chiefly
      // mobile, which never has the extension) a bare tap is the primary
      // gesture and must open the popup, not no-op.
      if (!/[，。！？、…\s]/.test(chars[lo])) {
